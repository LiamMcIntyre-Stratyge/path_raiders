...
- [x] **FND-03**: No privileged credentials ship in the client bundle (service-role key stays server-side; `.env.local` is gitignored; a CI/scan guard fails the build if a secret is bundled).
...
| FND-03 | Phase 9 — Backend Foundations & Integrity | Complete |
